...
void LR1Parser::buildParser(const std::string& grammarFile) {
    ...
    for (size_t i = 0; i < canonicalCollection.states.size(); ++i) {
        ...
        for (const auto& item : I.items) {
            if (item.dotPosition < item.rhs.size()) {
                ...
                // Reduce action - only on the lookahead terminal (not FOLLOW set)
                if (item.lhs == grammar.startSymbol && item.dotPosition == item.rhs.size()) {
                    // Accept
                    actionTable[{i, "$"}] = Action(ACCEPT, -1);
                } else {
                    // Reduce - only on the specific lookahead from the item
                    actionTable[{i, item.lookahead}] = Action(REDUCE, getProductionIndex(item));
                }
            }
        }
    }
...
std::string LR1Parser::joinRhs(const std::vector<std::string>& rhs) {
    std::string result;
...
bool LR1Parser::parse(const std::vector<std::string>& input) {
    Stack stack;
...
void LR1Parser::getProductionByIndex(int index, std::string& A, std::vector<std::string>& beta) {
...
void LR1Parser::printParsingTable() {
    std::cout << "LR(1) Parsing Table:" << std::endl;
    // Print header
...
void LR1Parser::printTrace(const std::vector<std::string>& input) {
